...
export function AnimatedCard({ className, ...props }: CardProps) {
  return (
    <div
      ...
      className={cn(
        "group/animated-card relative w-[356px] overflow-hidden rounded-3xl border border-border bg-card shadow-sm",
        className
      )}
      {...props}
    />
...
export function CardBody({ className, ...props }: CardProps) {
  return (
    <div
      role="group"
      className={cn(
        "flex flex-col space-y-1.5 border-t border-border p-5",
        className
      )}
      {...props}
    />
...
export function CardTitle({ className, ...props }: CardTitleProps) {
  return (
    <h3
      className={cn(
        "text-lg font-heading font-bold leading-none tracking-tight text-foreground",
        className
      )}
      {...props}
    />
...
export function CardVisual({ className, ...props }: CardProps) {
  return (
    <div
      className={cn("h-[180px] w-[356px] overflow-hidden bg-primary/5", className)}
      {...props}
    />
  );
}
...
export function Visual3({
  mainColor = "#005dac", // Strict deep blue
  secondaryColor = "#3385c6", // Lighter blue for contrast
  gridColor = "#005dac15", // Very faint blue grid
}: Visual3Props) {
  ...
  return (
    <>
      ...
      <div className="relative h-[180px] w-[356px] overflow-hidden rounded-t-3xl">
        <Layer4
          color={mainColor}
...
const Layer1: React.FC<LayerProps> = ({ color, secondaryColor }) => {
  return (
    <div
      className="absolute top-4 left-4 z-[8] flex items-center gap-2"
      style={
        {
          "--color": color,
          "--secondary-color": secondaryColor,
    ...
    >
      <div className="flex shrink-0 items-center rounded-full border border-primary/20 bg-background/80 px-2 py-1 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0 shadow-sm">
        <div className="h-1.5 w-1.5 rounded-full bg-[var(--color)]" />
        <span className="ml-1.5 text-[10px] font-mono font-bold tracking-wider text-foreground">
          80.17% AUC
        </span>
      </div>
      <div className="flex shrink-0 items-center rounded-full border border-primary/20 bg-background/80 px-2 py-1 backdrop-blur-sm transition-opacity duration-300 ease-in-out group-hover/animated-card:opacity-0 shadow-sm">
...
const Layer2: React.FC<{ color: string }> = ({ color }) => {
  return (
    ...
    >
      <div className="ease-out absolute inset-0 z-[7] flex w-[356px] translate-y-full items-start justify-center bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-0">
        <div className="ease-out rounded-xl border border-primary/20 bg-background/90 p-3 opacity-0 backdrop-blur-md transition-opacity duration-500 group-hover/animated-card:opacity-100 shadow-md">
          <div className="flex items-center gap-2 mb-1">
            <div className="h-2 w-2 shrink-0 rounded-full animate-pulse bg-[var(--color)]" />
            <p className="text-xs font-bold text-foreground font-heading">
              Gradient Boosting Engine
            </p>
          </div>
          <p className="text-[11px] text-muted-foreground font-mono leading-relaxed">
            Real-time inference on 14<br/>patient biometrics.
          </p>
        </div>
      </div>
    </div>
...
const Layer3: React.FC<{ color: string }> = ({ color }) => {
  return (
    <div className="ease-out absolute inset-0 z-[6] flex translate-y-full items-center justify-center opacity-0 transition-all duration-500 group-hover/animated-card:translate-y-0 group-hover/animated-card:opacity-100">
      <svg
        width="356"
        height="180"
        viewBox="0 0 356 180"
...
const Layer4: React.FC<LayerProps> = ({ color, secondaryColor, hovered }) => {
  ...
  return (
    <div className="ease-out absolute inset-0 z-[8] flex h-[180px] w-[356px] items-center justify-center text-primary/10 transition-transform duration-500 group-hover/animated-card:scale-150">
      <svg width="356" height="180" xmlns="http://www.w3.org/2000/svg">
        {rectsData.map((rect, index) => (
          <rect
            ...
            rx="2"
            ry="2"
            className="ease-out transition-all duration-500"
          />
        ))}
      </svg>
    </div>
